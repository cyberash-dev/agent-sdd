...
import {
	footprint,
	footprintEntriesForPath,
	impsMissingBinding,
} from "../../src/features/refresh/domain/Footprint.js";
...
test("binding tree walk collects string leaves", async () => {
	...
	const markdown = await readFile(
		join("tests", "fixtures", "spec.with-imps.md"),
		"utf8",
	);
	...
	assert.deepEqual(result.entries[0]?.paths, [
		"src/foo.ts",
		"src/bar.ts",
		"src/nested",
	]);
...
test("path can be covered by multiple IMP entries", async () => {
	...
	assert.deepEqual(
		entries.map((entry) => entry.impId),
		["fixture:IMP-001", "fixture:IMP-002"],
	);
});
