...
test("installRules all writes only under the agent-config home roots", async () => {
	...
	const fs = new FakeFs();
	const outcome = await installRules("all", false, {
		source: new FakeSource(MANIFEST, fullFiles()),
		fs,
	});
...
test("installRules all reports both targets and emits a valid action set", async () => {
	...
	for (const action of outcome.actions) {
		...
		assert.ok(
			["copy", "write_block", "merge_hook", "skip"].includes(action.op),
		);
...
test("installRules --dry-run plans actions but writes nothing", async () => {
	...
	const fs = new FakeFs();
	const outcome = await installRules("claude", true, {
		source: new FakeSource(MANIFEST, fullFiles()),
		fs,
	});
...
test("installRules fails with manifest-missing and writes nothing", async () => {
	...
	const fs = new FakeFs();
	const outcome = await installRules("claude", false, {
		source: new FakeSource(null, {}),
		fs,
	});
...
test("installRules fails with artifact-missing before any write (plan-then-apply)", async () => {
	...
	const files = {
		"a.md": "A",
		"skills/x/SKILL.md": null,
		"hooks/h.sh": "#!/bin/bash\n",
	};
	const outcome = await installRules("claude", false, {
		source: new FakeSource(MANIFEST, files),
		fs,
	});
