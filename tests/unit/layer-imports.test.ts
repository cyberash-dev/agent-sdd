...
test("source tree follows vertical-slice hexagonal import boundaries", async () => {
  ...
  for (const path of [join("src", "domain"), join("src", "ports"), join("src", "adapters"), join("src", "commands")]) {
...
function importViolation(file: string, specifier: string): string | undefined {
  ...
  if (specifier.startsWith("node:")) {
    ...
    const isAllowed = file === join("src", "shared", "domain", "Token.ts") && specifier === "node:crypto";
    return isAllowed ? undefined : `${file}: forbidden runtime import ${specifier}`;
  }
  ...
  if (source[1] === "shared") {
    return targetParts[1] === "features" ? `${file}: shared imports feature ${target}` : undefined;
  }
  ...
  if (layer === "application" && targetLayer !== "domain" && targetLayer !== "ports") {
    return `${file}: application imports ${target}`;
  }
  if (layer === "ports" && targetLayer !== "domain") {
    return `${file}: ports import ${target}`;
  }
  if (layer === "adapters" && targetLayer !== "application" && targetLayer !== "ports") {
