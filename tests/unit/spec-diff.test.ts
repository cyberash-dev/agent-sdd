...
test("classifyDiff: appending to schema.members is content_change (CTR-016 append-only)", () => {
	const prev = rec("p:CTR-16", "Contract", {
		schema: { members: { lint: ["sdd:a"], ready: ["x"] } },
	});
	const curr = rec("p:CTR-16", "Contract", {
		schema: { members: { lint: ["sdd:a"], ready: ["x", "y"] } },
	});
	const out = classifyDiff([prev], [curr]);
	assert.equal(out[0]!.classification, "content_change");
});
...
test("classifyDiff: adding a key inside a schema `fields` zone is content_change (CTR-012 append-only)", () => {
	const prev = rec("p:CTR-12", "Contract", {
		schema: {
			json: {
				lint: {
					type: "object",
					fields: { spec_files: {}, approver_blocklist: {} },
				},
			},
		},
	});
	const curr = rec("p:CTR-12", "Contract", {
		schema: {
			json: {
				lint: {
					type: "object",
...
test("classifyDiff: modifying an existing schema leaf outside a zone stays predicate_change", () => {
	const prev = rec("p:CTR-12", "Contract", {
		schema: { json: { lint: { type: "object", fields: { spec_files: {} } } } },
	});
	const curr = rec("p:CTR-12", "Contract", {
		schema: { json: { lint: { type: "array", fields: { spec_files: {} } } } },
	});
	const out = classifyDiff([prev], [curr]);
	assert.equal(out[0]!.classification, "predicate_change");
});
...
test("classifyDiff: adding an output field inside a schema `json` envelope is content_change (CTR-014 append-only)", () => {
	const prev = rec("p:CTR-14", "Contract", {
		schema: { json: { ok: "bool", violations: "array" } },
	});
	const curr = rec("p:CTR-14", "Contract", {
		schema: { json: { ok: "bool", violations: "array", advisories: "array" } },
	});
	const out = classifyDiff([prev], [curr]);
	assert.equal(out[0]!.classification, "content_change");
});
...
test("classifyDiff: removing a schema.members entry stays predicate_change", () => {
	const prev = rec("p:CTR-16", "Contract", {
		schema: { members: { ready: ["x", "y"] } },
	});
	const curr = rec("p:CTR-16", "Contract", {
		schema: { members: { ready: ["x"] } },
	});
	const out = classifyDiff([prev], [curr]);
	assert.equal(out[0]!.classification, "predicate_change");
});
...
test("classifyDiff: member append plus another schema field change stays predicate_change", () => {
	const prev = rec("p:CTR-16", "Contract", {
		schema: { grammar: "^a$", members: { ready: ["x"] } },
	});
	const curr = rec("p:CTR-16", "Contract", {
		schema: { grammar: "^b$", members: { ready: ["x", "y"] } },
	});
	const out = classifyDiff([prev], [curr]);
	assert.equal(out[0]!.classification, "predicate_change");
});
...
test("classifyDiff: lifecycle and approval_record changes ignored", () => {
	const prev = rec("p:CTR-1", "Contract", {
		schema: { x: 1 },
