...
import { parseManifest, ManifestError } from "../../src/features/install/domain/RuleManifest.js";
import { upsertManagedBlock, BLOCK_BEGIN, BLOCK_END } from "../../src/features/install/domain/ManagedBlock.js";
import { mergeHooks } from "../../src/features/install/domain/SettingsMerge.js";
import { buildPlan, type ExistingTargetFiles } from "../../src/features/install/domain/InstallPlan.js";
...
const EMPTY_EXISTING: ExistingTargetFiles = { claudeMd: null, settingsJson: null, agentsMd: null };
...
function sampleSources(): Map<string, string> {
  return new Map([["a.md", "A"], ["skills/x/SKILL.md", "S"], ["hooks/h.sh", "#!/bin/bash\n"]]);
}
...
test("parseManifest rejects a wrong format_version", () => {
  // @covers sdd-cli:CST-008
  assert.throws(() => parseManifest(JSON.stringify({ format_version: 2, artifacts: [] })), ManifestError);
});

test("parseManifest rejects an unknown artifact kind", () => {
  // @covers sdd-cli:CST-008
  const bad = JSON.stringify({ format_version: 1, artifacts: [{ source: "x", kind: "bogus", targets: ["claude"] }] });
...
test("parseManifest rejects a hook artifact without an event matcher", () => {
  // @covers sdd-cli:CST-008
  const bad = JSON.stringify({ format_version: 1, artifacts: [{ source: "h.sh", kind: "hook", targets: ["claude"] }] });
...
test("mergeHooks is idempotent and preserves a pre-existing user hook", () => {
  ...
  const settings = JSON.parse(twice) as { hooks: { PreToolUse: { matcher: string }[] } };
...
test("buildPlan for claude copies artifacts, writes the import block, and merges hooks", () => {
  // @covers sdd-cli:BEH-065
  const plan = buildPlan(sampleManifest(), "claude", sampleSources(), EMPTY_EXISTING, "/home/test");
  ...
  assert.ok(plan.writes.find((w) => w.absPath.endsWith("hooks/h.sh"))!.executable);
  assert.ok(plan.actions.some((a) => a.op === "merge_hook" && a.note === "Read"));
});

test("buildPlan for codex copies under .codex/sdd, writes AGENTS.md, and skips hooks", () => {
  // @covers sdd-cli:BEH-066
  const plan = buildPlan(sampleManifest(), "codex", sampleSources(), EMPTY_EXISTING, "/home/test");
  const paths = plan.writes.map((w) => w.absPath);
