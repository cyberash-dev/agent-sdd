...
import {
	matchId,
	rewriteApproval,
} from "../../src/features/approve/domain/Rewrite.js";
...
test("glob matches multiple records and rewrites each", () => {
	...
	assert.equal(
		(r.newContent.match(/lifecycle\.status: approved/g) ?? []).length,
		2,
	);
});
...
test("includes reviewed_test_oracle when provided", () => {
	...
	const r = rewriteApproval(
		md,
		{ ...REQ, id: "demo:srf-1", reviewedTestOracle: "tests/oracle.spec.ts" },
		FROZEN_TIME,
	);
...
test("rewrites nested lifecycle form already carrying approval_record placeholder", () => {
	...
	assert.equal(
		(result.newContent.match(/^ {2}approval_record:/gm) ?? []).length,
		1,
	);
	assert.match(result.newContent, /approver_identity: cyberash/);
});
...
test("glob enumerates only top-level records, not nested `- id:` items (INV-007)", () => {
	...
	assert.deepEqual(r.matched.map((m) => m.id).sort(), [
		"pol:audit",
		"pol:auth",
	]);
	assert.equal(
