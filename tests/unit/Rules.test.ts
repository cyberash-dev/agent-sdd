...
function record(partial: Partial<LintRecord>): LintRecord {
  return {
    ...
    parsed: { id: "demo:bar", template: "Behavior", "lifecycle.status": "proposed", version: 1 },
    file: "spec/demo.md",
    line: 10,
    rawBlock: "",
    ...partial,
...
test("lifecycleStatusRules: non-normative templates are skipped", () => {
  const out = lifecycleStatusRules(record({ template: "ImplementationBinding", lifecycleStatus: null }));
  assert.deepEqual(out, []);
});

test("approvalRecordRules: approved without real approval_record is an error", () => {
  const out = approvalRecordRules(record({
...
test("testObligationRules: normative ID with neither test_obligations nor aliased is an error", () => {
  const out = testObligationRules(record({ testObligations: [], hasAliasedObligations: false }));
  assert.equal(out.length, 1);
  assert.equal(out[0]!.rule, "sdd:test-obligation-required");
});

test("testObligationRules: aliased obligation field satisfies §4", () => {
  const out = testObligationRules(record({ testObligations: [], hasAliasedObligations: true }));
  assert.deepEqual(out, []);
});

test("testObligationRules: Surface template is exempt", () => {
  const out = testObligationRules(record({ template: "Surface", testObligations: [], hasAliasedObligations: false }));
  assert.deepEqual(out, []);
});

test("testObligationRules: missing on Constraint is a warning, not error", () => {
  const out = testObligationRules(record({ template: "Constraint", testObligations: [], hasAliasedObligations: false }));
  assert.equal(out.length, 1);
  assert.equal(out[0]!.severity, "warn");
});

test("fieldTypeRules: Invariant evidence enum", () => {
  const out = fieldTypeRules(record({
    template: "Invariant",
...
test("fieldTypeRules: integer version on a Behavior passes", () => {
  ...
  assert.equal(out.find((d) => d.rule === "sdd:type-version-int"), undefined);
});
