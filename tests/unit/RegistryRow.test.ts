...
test("reads compatible_sdd_cli from the ## Compatibility metadata table", () => {
	const out = parseRegistry(
		doc([
			"| ENF-001 | — | weasel words | structural-lint | sdd lint | spec-valid | sdd:weasel-word | implemented | — | — |",
		]),
	);
...
test("treats an em-dash diagnostic_id cell as no diagnostic id", () => {
	const out = parseRegistry(
		doc([
			"| ENF-007A | ENF-007 | report obligations | report-evidence | sdd report | implementation-valid | — | implemented | PR author | PR merge |",
		]),
	);
	assert.ok(out.ok);
	assert.deepEqual(out.doc.rows[0]!.diagnosticIds, []);
...
test("treats an `implemented:hybrid` maturity suffix as implemented", () => {
	const out = parseRegistry(
		doc([
			"| ENF-020B | ENF-020 | debt monotonicity | aggregate-gate | sdd ready | implementation-valid | debt_budget_increased | implemented:hybrid | partition owner | sprint close |",
		]),
	);
	assert.ok(out.ok);
	assert.equal(out.doc.rows.length, 1);
