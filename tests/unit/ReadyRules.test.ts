...
function record(overrides: Partial<LintRecord> & { id: string; template: string; lifecycleStatus: string | null }): LintRecord {
  return {
    id: overrides.id,
...
function viewWith(records: LintRecord[], creditedById: Record<string, Marker[]> = {}): PartitionView {
...
test("ruleUnapproved fires for a proposed normative ID outside sandbox_paths", () => {
  ...
  const view = viewWith([record({ id: "fixture:BEH-001", template: "Behavior", lifecycleStatus: "proposed", file: "spec/spec.md" })]);
...
test("ruleUnapproved skips proposed IDs inside sandbox_paths", () => {
  ...
  const view = viewWith([record({ id: "fixture:BEH-001", template: "Behavior", lifecycleStatus: "proposed", file: "spike/draft.md" })]);
...
test("ruleUncovered exempts approved IDs with not_applicable test_obligation", () => {
  ...
  const view = viewWith([record({
    ...
    template: "Behavior",
    lifecycleStatus: "approved",
    parsed: { test_obligation: { not_applicable: "doc_only", reason: "descriptive" } },
  })]);
...
test("ruleRemovedNoCompatTest fires when no marker carries a compatibility_action token", () => {
  // @covers sdd-cli:BEH-018
  const view = viewWith(
    [record({ id: "fixture:BEH-001", template: "Behavior", lifecycleStatus: "removed", parsed: { compatibility_action: "reject" } })],
    { "fixture:BEH-001": [{ partition: "fixture", id: "BEH-001", tail: {}, file: "tests/foo.test.ts", line: 1 }] },
  );
...
test("ruleRemovedCompatActionMismatch reports expected/actual when tokens differ", () => {
  ...
  const view = viewWith(
    [record({ id: "fixture:BEH-001", template: "Behavior", lifecycleStatus: "removed", parsed: { compatibility_action: "migrate" } })],
    {
      "fixture:BEH-001": [{
        partition: "fixture",
        id: "BEH-001",
...
test("ruleSurfaceUnapprovedRef fires when an approved Surface references a proposed member", () => {
  ...
  const member = record({ id: "fixture:CTR-001", template: "Contract", lifecycleStatus: "proposed" });
...
test("ruleSurfaceUnapprovedRef does not fire when all members are approved", () => {
  ...
  const member = record({ id: "fixture:CTR-001", template: "Contract", lifecycleStatus: "approved" });
  const view = viewWith([surface, member]);
...
test("ruleOrphanCovers fires when marker partition is configured but ID is missing", () => {
  ...
  const scanned: ScannedMarker[] = [{
    marker: { partition: "fixture", id: "BEH-999", tail: {}, file: "tests/foo.ts", line: 1 },
...
test("ruleOrphanCovers does NOT double-fire when partition is unconfigured (deferred to unknown_partition)", () => {
  // @covers sdd-cli:BEH-018
  const scanned: ScannedMarker[] = [{
    marker: { partition: "ghost", id: "BEH-001", tail: {}, file: "tests/foo.ts", line: 1 },
