...
import { inspectBody, normalizeBody } from "../../src/features/record/domain/RecordBody.js";

test("normalizeBody: bare body is returned unchanged (trimmed)", () => {
  // @covers sdd-cli:BEH-059
  assert.equal(normalizeBody("id: x:A-1\ntype: Thing\n"), "id: x:A-1\ntype: Thing");
});

test("normalizeBody: strips an enclosing yaml fence", () => {
  // @covers sdd-cli:BEH-059
  const fenced = ["```yaml", "---", "id: x:A-1", "type: Thing", "---", "```"].join("\n");
...
test("inspectBody: extracts id and lifecycle status", () => {
  ...
  const body = ["id: x:A-1", "type: Behavior", "lifecycle:", "  status: proposed"].join("\n");
