...
async function sectionDiagnosticsByFile(
	root: string,
	...
	for (const d of body.diagnostics) {
		if (d.rule !== "sdd:section-presence" && d.rule !== "sdd:section-order")
			continue;
...
test("BEH-052: partition_glob restricts the §2 check to matching files", async () => {
	...
	assert.ok(
		(counts.get("spec/partition.md") ?? 0) > 0,
		"matching file should get the §2 check",
	);
	assert.equal(
		counts.get("spec/other.md") ?? 0,
		0,
		"non-matching file must be exempt even with a ## 1. Context heading",
	);
});
...
test("BEH-052: absent partition_glob falls back to heading-based detection", async () => {
	...
	const counts = await sectionDiagnosticsByFile(root);
	assert.ok(
		(counts.get("spec/other.md") ?? 0) > 0,
		"heading-shaped file should get the §2 check under the fallback",
	);
});
