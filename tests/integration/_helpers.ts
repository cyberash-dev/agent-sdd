...
export async function fixtureRepo(): Promise<FixtureRepo> {
	...
	const tokenBody = JSON.parse(tokenResult.stdout) as {
		token: string;
		commit_sha: string;
	};
	await writeFile(
		join(root, "spec", "spec.md"),
		specText(tokenBody.token, tokenBody.commit_sha),
	);
	await git(root, ["add", "spec/spec.md"]);
	await commit(root, "record baseline");
	return {
		root,
		baselineCommitSha: tokenBody.commit_sha,
		baselineToken: tokenBody.token,
	};
}
