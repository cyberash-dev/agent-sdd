...
import { commit, fixtureRepo, git, projectRoot, runSdd, specText } from "./_helpers.js";
...
test("token emits current scope token as JSON", async () => {
  ...
  const body = JSON.parse(result.stdout) as { ok: boolean; token: string; mechanism: string; scope: string[] };
...
test("token treats untracked scope file as dirty", async () => {
  ...
  const repo = await fixtureRepo();
  await writeFile(join(repo.root, "src", "new.ts"), "export const value = 1;\n");

  const result = await runSdd(repo.root, ["token", "--format=json"]);
  const body = JSON.parse(result.stdout) as { ok: boolean; reason: string; dirty_paths: string[] };
...
test("token treats tracked scope file modification as dirty", async () => {
  // @covers sdd-cli:BEH-002
  const repo = await fixtureRepo();
  await writeFile(join(repo.root, "src", "foo.ts"), "export const value = 99;\n");
...
test("check exits zero when recorded token matches", async () => {
  ...
  const body = JSON.parse(result.stdout) as { ok: boolean; recorded_token: string; baseline_commit_sha: string };
...
test("check reports baseline-stale after a scope file is modified and committed", async () => {
  ...
  const repo = await fixtureRepo();
  await writeFile(join(repo.root, "src", "foo.ts"), "export const value = 2;\n");
...
test("check reports duplicate baseline block as config error", async () => {
  ...
  const repo = await fixtureRepo();
  await writeFile(join(repo.root, "spec", "spec.md"), `${await readFile(join(repo.root, "spec", "spec.md"), "utf8")}\n${specText(repo.baselineToken, repo.baselineCommitSha)}`);
...
test("config schema violation (missing discovery_scope) is a config error", async () => {
  ...
  const repo = await fixtureRepo();
  await writeFile(join(repo.root, ".sdd", "config.json"), JSON.stringify({
...
test("missing baseline block in spec is a config error", async () => {
  // @covers sdd-cli:BEH-009
  const repo = await fixtureRepo();
  await writeFile(join(repo.root, "spec", "spec.md"), "# fixture\n\nno blocks here\n");
...
test("PATH lacks git is an environment error", async () => {
  ...
  const result = await runSdd(repo.root, ["token", "--format=json"], { env: { PATH: "" } });
  const body = JSON.parse(result.stdout) as { reason: string };
...
test("refresh emits Delta and Open-Q stubs for drift", async () => {
  ...
  const repo = await fixtureRepo();
  await writeFile(join(repo.root, "src", "foo.ts"), "export const value = 3;\n");
  await writeFile(join(repo.root, "src", "notes.ts"), "export const note = 1;\n");
...
test("refresh includes uncommitted scope drift", async () => {
  // @covers sdd-cli:BEH-006
  const repo = await fixtureRepo();
  await writeFile(join(repo.root, "src", "foo.ts"), "export const value = 4;\n");

  const result = await runSdd(repo.root, ["refresh", "--format=json"]);
  const body = JSON.parse(result.stdout) as { stubs: Array<{ kind: string; path: string }> };

  assert.equal(result.code, 0);
  assert.deepEqual(body.stubs.map((stub) => stub.path), ["src/foo.ts"]);
...
test("refresh emits empty JSON list with no drift", async () => {
  ...
  const result = await runSdd(repo.root, ["refresh", "--format=json"]);
  const body = JSON.parse(result.stdout) as { format_version: number; stubs: unknown[] };
...
test("refresh human format omits emitted_at", async () => {
  ...
  const repo = await fixtureRepo();
  await writeFile(join(repo.root, "src", "foo.ts"), "export const value = 5;\n");
  await git(repo.root, ["add", "src/foo.ts"]);
