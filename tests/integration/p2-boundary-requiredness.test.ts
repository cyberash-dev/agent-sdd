...
test("BEH-034: boundary CTR with empty policy_refs triggers", async () => {
  const { root } = await fixtureProject(fixture({
    applicability: true,
  ...
  assert.equal(code, 1);
  const fired = body.diagnostics.filter((d) => d.rule === "sdd:boundary-policy-ref");
...
test("BEH-035: boundary CTR without concurrency_model triggers", async () => {
  ...
  const { body } = await lintBody(root);
  const fired = body.diagnostics.filter((d) => d.rule === "sdd:boundary-concurrency-model");
  assert.equal(fired.length, 1);
});
...
test("BEH-036: boundary CTR without applicability triggers", async () => {
  ...
  const { body } = await lintBody(root);
  const fired = body.diagnostics.filter((d) => d.rule === "sdd:applicability-required");
  assert.equal(fired.length, 1);
});
...
test("BEH-037: boundary CTR without data_scope triggers", async () => {
  ...
  const { body } = await lintBody(root);
  const fired = body.diagnostics.filter((d) => d.rule === "sdd:data-scope-required");
  assert.equal(fired.length, 1);
});
...
test("non-boundary CTR (no surface_ref to external Surface) does NOT fire any boundary rule", async () => {
  ...
  const { body } = await lintBody(root);
  const ruleIds = ["sdd:boundary-policy-ref", "sdd:boundary-concurrency-model", "sdd:applicability-required", "sdd:data-scope-required"];
  const fired = body.diagnostics.filter((d) => ruleIds.includes(d.rule));
  assert.deepEqual(fired, [], `non-boundary record incorrectly triggered: ${JSON.stringify(fired)}`);
});
