...
test("approve default mode writes attestation to .sdd/plans/<id>.yaml without touching spec (BEH-021, INV-011)", async () => {
	...
	const active = (
		await readFile(join(root, ".sdd", "plans", ".active"), "utf8")
	).trim();
...
test("approve --inline preserves legacy direct rewrite and emits stderr deprecation warning (BEH-022)", async () => {
	...
	assert.match(
		result.stderr,
		/DEPRECATED: --inline will be removed in v1\.1\.0/,
	);
...
test("finalize materialises every attestation atomically and archives the plan (BEH-024, INV-012)", async () => {
	...
	const planId = (JSON.parse(approveJson.stdout) as { plan_id: string })
		.plan_id;
	...
	assert.equal(
		await exists(join(root, ".sdd", "plans", `${planId}.yaml`)),
		false,
	);
	assert.equal(
		await exists(join(root, ".sdd", "plans", "finalized", `${planId}.yaml`)),
		true,
	);
...
test("finalize refuses on graph violation, leaves spec byte-stable (BEH-025, INV-012)", async () => {
	...
	assert.equal(
		await exists(join(root, ".sdd", "plans", `${active}.yaml`)),
		true,
	);
	assert.equal(
...
test("approve queues multiple attestations into the same active plan", async () => {
	...
	assert.equal(
		planId1,
		planId2,
		"both attestations must land in the same plan",
	);
	...
	assert.deepEqual(showBody.plan.pending_attestations.map((a) => a.id).sort(), [
		"fixture:beh-1",
		"fixture:beh-2",
	]);
});
...
test("finalize flips a record whose lifecycle follows a nested `- id:` list (INV-007, BEH-024)", async () => {
	...
	const body = JSON.parse(finalize.stdout) as {
		ok: boolean;
		finalized_ids: string[];
		files_changed: string[];
	};
