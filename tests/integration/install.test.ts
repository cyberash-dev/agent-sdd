...
test("sdd install claude installs rules, skill, import block, and both hooks", async () => {
	...
	const result = await runSdd(process.cwd(), ["install", "claude"], {
	...
	assert.ok(
		await exists(join(home, ".claude/skills/spec-driven-development/SKILL.md")),
	);
...
test("sdd install claude preserves a pre-existing user hook in settings.json", async () => {
	// @covers sdd-cli:BEH-065
	const home = await tmpHome();
	await runSdd(process.cwd(), ["install", "claude"], {
		env: { SDD_INSTALL_HOME: home },
	});
	// Re-run after a user hand-added an unrelated hook; the installer must keep it.
	const settingsPath = join(home, ".claude/settings.json");
	const withUser = JSON.parse(await readFile(settingsPath, "utf8")) as {
		hooks: { PreToolUse: unknown[] };
	};
	withUser.hooks.PreToolUse.push({
		matcher: "Bash",
		hooks: [{ type: "command", command: "/user/custom.sh" }],
	});
	await (
		await import("node:fs/promises")
	).writeFile(settingsPath, JSON.stringify(withUser), "utf8");
...
test("sdd install all installs both targets", async () => {
	...
	const result = await runSdd(process.cwd(), ["install", "all"], {
		env: { SDD_INSTALL_HOME: home },
	});
...
test("sdd install --dry-run writes nothing and reports dry_run true", async () => {
	...
	const result = await runSdd(
		process.cwd(),
		["install", "all", "--dry-run", "--format=json"],
		{ env: { SDD_INSTALL_HOME: home } },
	);
...
test("sdd install JSON output matches the CTR-030 envelope", async () => {
	...
	for (const action of body.actions) {
		assert.ok(
			typeof action.target === "string" && typeof action.kind === "string",
		);
...
test("sdd install rejects a missing or unknown target with exit 2", async () => {
	...
	const unknownFlag = await runSdd(
		process.cwd(),
		["install", "claude", "--nope"],
		{ env },
	);
...
test("sdd install never writes inside the repo working tree", async () => {
	...
	const result = await runSdd(repo, ["install", "all"], {
		env: { SDD_INSTALL_HOME: home },
	});
