...
export async function readyFixture(options: ReadyFixtureOptions): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "sdd-ready-"));
  await mkdir(join(root, ".sdd"));
  await writeFile(join(root, ".sdd", "config.json"), JSON.stringify(options.config, null, 2));
...
export async function runReady(cwd: string, extraArgs: readonly string[] = []): Promise<RunResult> {
