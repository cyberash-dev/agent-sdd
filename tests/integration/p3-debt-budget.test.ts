...
const partitionWithBudget = (current: number, trend: string, baselineAt = "2026-04-01"): string => [
  "```yaml",
  "---",
  "id: fixture:PRT-1",
  "type: Partition",
...
test("BEH-042: Partition without unmodeled_budget triggers sdd:debt-budget-form", async () => {
  ...
  assert.equal(code, 1);
  const fired = body.diagnostics.filter((d) => d.rule === "sdd:debt-budget-form");
...
test("BEH-042: Partition with negative current triggers sdd:debt-budget-form", async () => {
  const { root } = await fixtureProject(partitionWithBudget(-1, "monotonic_non_increasing"));
...
test("BEH-042: Partition with invalid trend triggers sdd:debt-budget-form", async () => {
  const { root } = await fixtureProject(partitionWithBudget(0, "wishy_washy_trend"));
  const { body } = await lintBody(root);
  const fired = body.diagnostics.filter((d) => d.rule === "sdd:debt-budget-form");
  assert.equal(fired.length, 1);
...
test("BEH-042: Partition with all sub-fields valid is silent", async () => {
  const { root } = await fixtureProject(partitionWithBudget(5, "monotonic_non_increasing"));
  const { body } = await lintBody(root);
  const fired = body.diagnostics.filter((d) => d.rule === "sdd:debt-budget-form");
...
async function fixtureGitProject(initialBudget: number, trend: string): Promise<{ root: string; baseSha: string }> {
  const { root } = await fixtureProject(partitionWithBudget(initialBudget, trend));
  await gitInit(root);
...
test("BEH-043: monotonic_non_increasing — current grew → fires debt_budget_increased", async () => {
  const { root, baseSha } = await fixtureGitProject(5, "monotonic_non_increasing");
  ...
  const body = JSON.parse(r.stdout) as { violations: Array<{ kind: string; id?: string; expected?: string; actual?: string }> };
  const fired = body.violations.filter((v) => v.kind === "debt_budget_increased");
...
test("BEH-043: monotonic_decreasing — current unchanged → fires (strict trend)", async () => {
  ...
  const r = await runSdd(root, ["ready", "--against", baseSha, "--format=json"]);
  const body = JSON.parse(r.stdout) as { violations: Array<{ kind: string; expected?: string; actual?: string }> };
  const fired = body.violations.filter((v) => v.kind === "debt_budget_increased");
  assert.equal(fired.length, 1);
  assert.equal(fired[0]!.expected, "< 5");
});
