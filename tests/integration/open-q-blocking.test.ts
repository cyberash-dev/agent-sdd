...
async function openQBlockingFired(root: string): Promise<{ code: number; fired: LintBody["diagnostics"] }> {
  const r = await runSdd(root, ["lint", "--format=json"]);
  const body = JSON.parse(r.stdout) as LintBody;
  return { code: r.code, fired: body.diagnostics.filter((d) => d.rule === "sdd:open-q-blocking") };
}

function openQBlock(id: string, opts: { blocking: string; status?: string }): string {
  return [
    "```yaml",
    "---",
    `id: ${id}`,
...
test("BEH-050: Open-Q with blocking=yes triggers sdd:open-q-blocking", async () => {
  const { root } = await fixtureProject(openQBlock("fixture:oq-1", { blocking: "yes" }));
...
test("BEH-050: Open-Q with blocking=no is silent", async () => {
  const { root } = await fixtureProject(openQBlock("fixture:oq-2", { blocking: "no" }));
...
test("BEH-050: Open-Q with blocking=yes but status=removed is silent", async () => {
  const { root } = await fixtureProject(openQBlock("fixture:oq-3", { blocking: "yes", status: "removed" }));
