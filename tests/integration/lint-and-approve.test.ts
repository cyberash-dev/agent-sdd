...
test("sdd lint exits 1 when an error rule fires", async () => {
	...
	assert.ok(
		body.diagnostics.some(
			(d) =>
				d.rule === "sdd:approval-record-required" && d.severity === "error",
		),
	);
});
...
test("sdd approve includes reviewed_test_oracle when provided", async () => {
	...
	const result = await runSdd(root, [
		...
		"--reviewed-test-oracle",
		"tests/oracle.spec.ts",
	]);
	const specText = await readFile(join(root, "spec", "spec.md"), "utf8");
...
test("sdd approve inserts approval_record when source has no placeholder (INV-007)", async () => {
	...
	assert.ok(
		!lintBody.diagnostics.some(
			(d) =>
				d.rule === "sdd:approval-record-required" && d.id === "fixture:ext-1",
		),
...
test("sdd approve rewrites multiple records when --id is a glob", async () => {
	...
	assert.equal(
		(specText.match(/lifecycle\.status: approved/g) ?? []).length,
		2,
	);
});
