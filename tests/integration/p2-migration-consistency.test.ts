...
const MIGRATION = (
	...
	opts: {
		...
		targetIds?: string[];
		partitionSlice?: Array<{ coordinator: boolean }>;
	},
): string => {
	const lines: string[] = [];
	lines.push("```yaml");
	...
	if (opts.partitionSlice !== undefined) {
		...
		for (let i = 0; i < opts.partitionSlice.length; i++) {
			...
			if (slice.coordinator)
				lines.push(`    coordinator_id: fixture:COORD-${i}`);
		}
	}
...
test("BEH-038: Invariant referencing a missing Migration triggers", async () => {
	const { root } = await fixtureProject(
		INVARIANT_DEPENDS_ON_MIGRATION("fixture:MIG-MISSING"),
	);
	const { code, body } = await lintBody(root);
	assert.equal(code, 1);
	const fired = body.diagnostics.filter(
		(d) => d.rule === "sdd:migration-enforcement-stage",
	);
...
test("BEH-039: cross-partition Migration without partition_slice triggers", async () => {
	...
	const { body: res } = await lintBody(root);
	const fired = res.diagnostics.filter(
		(d) => d.rule === "sdd:migration-cross-partition",
	);
	assert.equal(fired.length, 1);
