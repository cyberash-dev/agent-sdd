...
test("BEH-059 / CTR-028: record set replaces a proposed record body (--content)", async () => {
  ...
  const r = await runSdd(root, ["record", "set", "fixture:BEH-001", "--content", UPDATED_BEH_001, "--format=json"]);

  assert.equal(r.code, 0);
  const body = JSON.parse(r.stdout) as { ok: boolean; action: string; id: string };
...
test("BEH-059: record set accepts a fenced body via --from-file", async () => {
  // @covers sdd-cli:BEH-059
  const root = await editFixture();
  const fenced = ["```yaml", "---", "id: fixture:BEH-001", "type: Behavior", "lifecycle:", "  status: proposed", "title: fromfile", "---", "```"].join("\n");
  await writeFile(join(root, "body.yaml"), fenced);

  const r = await runSdd(root, ["record", "set", "fixture:BEH-001", "--from-file", "body.yaml"]);

  assert.equal(r.code, 0);
  const got = await runSdd(root, ["record", "get", "fixture:BEH-001"]);
  assert.equal(got.stdout.trimEnd(), ["id: fixture:BEH-001", "type: Behavior", "lifecycle:", "  status: proposed", "title: fromfile"].join("\n"));
});
...
test("BEH-060 / INV-015: record set refuses an approved record and writes nothing", async () => {
  ...
  const newBody = ["id: fixture:INV-001", "type: Invariant", "lifecycle:", "  status: proposed", "never: changed"].join("\n");

  const r = await runSdd(root, ["record", "set", "fixture:INV-001", "--content", newBody]);
...
test("BEH-060: record set refuses a body that declares a protected status", async () => {
  ...
  const root = await editFixture();
  const before = await readFile(join(root, "spec", "spec.md"), "utf8");
  const newBody = ["id: fixture:BEH-001", "type: Behavior", "lifecycle:", "  status: approved", "title: sneak"].join("\n");

  const r = await runSdd(root, ["record", "set", "fixture:BEH-001", "--content", newBody]);
...
test("BEH-061: record set on an unknown id exits 1 and writes nothing", async () => {
  ...
  const root = await editFixture();
  const before = await readFile(join(root, "spec", "spec.md"), "utf8");
  const newBody = ["id: fixture:NOPE-9", "type: Behavior", "lifecycle:", "  status: proposed"].join("\n");

  const r = await runSdd(root, ["record", "set", "fixture:NOPE-9", "--content", newBody]);
...
test("BEH-064: record set with id/body mismatch exits 2", async () => {
  ...
  const r = await runSdd(root, ["record", "set", "fixture:BEH-001", "--content", "id: fixture:OTHER-1\ntype: Behavior"]);
...
test("BEH-062 / CTR-028: record add inserts a new record after the anchor", async () => {
  ...
  const r = await runSdd(root, ["record", "add", "--after", "fixture:BEH-001", "--content", NEW_BEH_002, "--format=json"]);
...
test("BEH-063: record add refuses a duplicate id and writes nothing", async () => {
  ...
  const root = await editFixture();
  const before = await readFile(join(root, "spec", "spec.md"), "utf8");
  const dup = ["id: fixture:BEH-001", "type: Behavior", "lifecycle:", "  status: proposed"].join("\n");

  const r = await runSdd(root, ["record", "add", "--after", "fixture:BEH-001", "--content", dup]);
...
test("BEH-063: record add refuses an unknown anchor", async () => {
  ...
  const r = await runSdd(root, ["record", "add", "--after", "fixture:NOPE-9", "--content", NEW_BEH_002]);
...
test("INV-015: record set rewrites only the target spec file", async () => {
  ...
  const bContent = ["# other", "", "```yaml", "---", "id: other:BEH-001", "type: Behavior", "lifecycle:", "  status: proposed", "---", "```", ""].join("\n");
  await writeFile(join(root, "spec", "b.md"), bContent);
  const configBefore = await readFile(join(root, ".sdd", "config.json"), "utf8");

  const r = await runSdd(root, ["record", "set", "fixture:BEH-001", "--content", UPDATED_BEH_001]);

  assert.equal(r.code, 0);
  assert.equal(await readFile(join(root, "spec", "b.md"), "utf8"), bContent);
  assert.equal(await readFile(join(root, ".sdd", "config.json"), "utf8"), configBefore);
});
