...
export interface PartitionView {
  ...
  recordsById: Map<string, LintRecord>;  // key = full id, e.g. "sdd-cli:BEH-001"
  creditedMarkersById: Map<string, Marker[]>; // markers in partition.test_paths whose prefix == partition.name
}
...
export function ruleUnapproved(view: PartitionView): ReadyViolation[] {
  ...
  for (const rec of view.records) {
    if (!isNormative(rec)) continue;
    const status = rec.lifecycleStatus;
    if (status !== "draft" && status !== "proposed") continue;
    if (fileInGlobs(rec.file, view.partition.sandboxPaths)) continue;
    out.push({
      kind: "unapproved",
      id: rec.id,
...
export function ruleUncovered(view: PartitionView): ReadyViolation[] {
  ...
  for (const rec of view.records) {
    if (!isNormative(rec)) continue;
    if (rec.template !== null && COVERAGE_EXEMPT_TEMPLATES.has(rec.template)) continue;
    if (rec.lifecycleStatus !== "approved" && rec.lifecycleStatus !== "deprecated") continue;
    if (isNotApplicableTestObligation(rec)) continue;
    if ((view.creditedMarkersById.get(rec.id) ?? []).length > 0) continue;
    out.push({
      kind: "uncovered",
      id: rec.id,
      partition: view.partition.name,
      ...
      file: rec.file,
      line: rec.line,
      remediation: "add `@covers " + rec.id + "` next to a test for this ID, or set `Test obligation: not_applicable + reason`",
    });
  }
  return out;
}
...
export function ruleRemovedNoCompatTest(view: PartitionView): ReadyViolation[] {
  ...
  for (const rec of view.records) {
    if (!isNormative(rec)) continue;
    if (rec.lifecycleStatus !== "removed") continue;
    ...
    out.push({
      ...
      file: rec.file,
      line: rec.line,
      remediation: "add a test with `@covers " + rec.id + " compatibility_action=<action>`",
    });
  }
  return out;
}

export function ruleRemovedCompatActionMismatch(view: PartitionView): ReadyViolation[] {
  const out: ReadyViolation[] = [];
  for (const rec of view.records) {
    if (!isNormative(rec)) continue;
    if (rec.lifecycleStatus !== "removed") continue;
...
export function ruleSurfaceUnapprovedRef(view: PartitionView): ReadyViolation[] {
  const out: ReadyViolation[] = [];
  for (const rec of view.records) {
    if (rec.template !== "Surface") continue;
    const status = rec.lifecycleStatus;
    if (status !== "approved" && status !== "deprecated" && status !== "removed") continue;
    const members = readMembers(rec);
    for (const memberId of members) {
      const member = view.recordsById.get(memberId);
      if (member === undefined) continue;
      const ms = member.lifecycleStatus;
      if (ms === "approved" || ms === "deprecated" || ms === "removed") continue;
      out.push({
...
export function ruleOrphanCovers(scanned: readonly ScannedMarker[]): ReadyViolation[] {
  const out: ReadyViolation[] = [];
  for (const s of scanned) {
    if (!s.isPartitionConfigured) continue; // covered by ruleUnknownPartitionCovers
    if (s.hasMatchingRecord) continue;
    out.push({
      kind: "orphan_covers",
...
export function ruleUnknownPartitionCovers(scanned: readonly ScannedMarker[]): ReadyViolation[] {
  const out: ReadyViolation[] = [];
  for (const s of scanned) {
    if (s.isPartitionConfigured) continue;
    out.push({
...
function isNotApplicableTestObligation(rec: LintRecord): boolean {
  ...
  if (isObject(v) && v.not_applicable !== undefined) return true;
  return false;
}
