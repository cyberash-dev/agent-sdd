...
export class ChildProcessReadyGit implements ReadyGitPort {
	...
	async treeBytes(
		...
		if (result.code !== 0) {
			throw new Error(
				result.stderr.toString("utf8").trim() || "git ls-tree failed",
			);
		}
	...
	async dirtyPaths(
		...
		if (diffResult.code !== 0 && diffResult.code !== 1) {
			throw new Error(
				diffResult.stderr.toString("utf8").trim() || "git diff --quiet failed",
			);
		}
		const result = await runGit(repoRoot, [
			"status",
		...
		if (result.code !== 0) {
			throw new Error(
				result.stderr.toString("utf8").trim() || "git status failed",
			);
		}
		return nonEmptyLines(result.stdout.toString("utf8"))
