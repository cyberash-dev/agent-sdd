...
export class CliReadyHandler implements ReadyCommand {
	...
	async execute(
		...
		try {
			envelope = await runReady(
				cwd,
				{ partitionFilter: partition, against },
				this.ports,
			);
		} catch (error) {
			/*
		...
		const exitCode =
			envelope.error !== null ? 2 : envelope.violations.length > 0 ? 1 : 0;
		if (format === "json") {
...
function humanResult(
	...
	for (const a of envelope.advisories) {
		lines.push(
			`[advisory:${a.kind}] ${a.file}:${a.line}  ${truncate(a.remediation, 200)}`,
		);
	}
...
function defaultRemediation(v: ReadyViolation): string {
	if (
		v.kind === "removed_compat_action_mismatch" &&
