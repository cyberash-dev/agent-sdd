...
export class NodeFinalizeFileSystem implements FinalizeConfigPort, FinalizeFileSystem {
  async config(repoRoot: string): Promise<SddConfig> {
    ...
    try {
      ...
      throw configFailure("config-invalid", `JSON parse error: ${(e as Error).message}`, undefined, configPath);
    }
  ...
  async writeBatch(repoRoot: string, entries: ReadonlyArray<{ path: string; content: string }>): Promise<void> {
...
async function walk(baseDir: string, segments: readonly string[]): Promise<string[]> {
  ...
  for (const e of entries) {
    ...
    if (rest.length === 0) {
      if (await isFile(next)) out.push(next);
      continue;
    }
    if (e.isDirectory()) {
