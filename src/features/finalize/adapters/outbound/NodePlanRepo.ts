...
export class NodePlanRepo implements PlanRepo {
  async load(repoRoot: string, plansDir: string, planId: string | undefined): Promise<PlanLoad> {
    const dirAbs = resolve(repoRoot, plansDir);
    const id = planId ?? (await readActive(dirAbs));
    if (id === null) return { kind: "no-active-plan" };
    ...
    try {
      ...
      return { kind: "invalid-plan-file", planId: id, sourcePath: sourceRel, reason: "plan file not found" };
    }
    let parsed: unknown;
    try {
  ...
  async archive(repoRoot: string, plansDir: string, planId: string): Promise<{ archivedPath: string }> {
    const dirAbs = resolve(repoRoot, plansDir);
