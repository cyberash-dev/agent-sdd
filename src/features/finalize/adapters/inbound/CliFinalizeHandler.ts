...
import { runFinalize, type RunFinalizePorts, type RunFinalizeOutcome } from "../../application/RunFinalize.js";
...
export class CliFinalizeHandler implements FinalizeCommand {
  ...
  async execute(cwd: string, req: FinalizeRequest, format: Exclude<OutputFormat, "yaml">): Promise<CommandResult> {
    const outcome = await runFinalize(cwd, req.planId, this.ports);
    return render(outcome, format);
  }
}

function render(outcome: RunFinalizeOutcome, format: Exclude<OutputFormat, "yaml">): CommandResult {
  if (outcome.kind === "finalized") {
    ...
    for (const id of outcome.finalizedIds) lines.push(`  - ${id}`);
    lines.push("");
