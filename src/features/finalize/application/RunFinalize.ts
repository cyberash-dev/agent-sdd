import { rewriteApproval, type ApprovalAttestation } from "../../../shared/domain/SpecApprovalRewrite.js";
import { lintRecordsFromMarkdown } from "../../../shared/domain/SpecRecord.js";
import { validateFinalizeGraph, type GraphViolation } from "../domain/ValidateFinalizeGraph.js";
...
export async function runFinalize(
  ...
  if (load.kind === "no-active-plan") return { kind: "no-active-plan" };
  if (load.kind === "invalid-plan-file") {
    return { kind: "invalid-plan", planId: load.planId, sourcePath: load.sourcePath, reason: load.reason };
  }
  ...
  // Apply each attestation as a rewriteApproval against the matching record.
  // We collect per-file new content; if any attestation fails to match an
  // existing record, abort before writing (atomicity per INV-012).
  const when = ports.clock.now();
  const fileContent = new Map<string, string>();
  for (const e of entries) fileContent.set(e.path, e.content);
  ...
  for (const a of plan.pendingAttestations) {
    ...
    for (const e of entries) {
      ...
      matched = true;
      for (const m of result.matched) finalizedIds.push(m.id);
      break;
    }
    if (!matched) missingIds.push(a.id);
  }

  if (missingIds.length > 0) {
    return { kind: "no-id-match", planId: plan.planId, missingIds };
  }
