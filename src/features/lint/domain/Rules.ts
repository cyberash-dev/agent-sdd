...
import { NORMATIVE_TEMPLATES, VALID_LIFECYCLE_STATUS, type LintRecord } from "./Record.js";
...
export function fieldTypeRules(rec: LintRecord): Diagnostic[] {
  ...
  // version — int (Surface is exempt: SDD discipline declares Surface the
  // unit of semver, with consumer_compat_policy: semver_per_surface — its
  // version is a semver string like "0.1.0", not an integer)
...
export interface WeaselFinding {
  line: number;       // 1-based
  word: string;
  section: string;    // heading the line belongs to, or "" if outside any
}
