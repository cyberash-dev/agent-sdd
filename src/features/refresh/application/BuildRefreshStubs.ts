...
export async function buildRefreshStubs(
	...
	const missingBinding = impsMissingBinding(
		loadedSpec.blocks,
		config.footprint.bindingIdPrefix,
		config.footprint.bindingField,
	);
	...
	const committedPaths = await ports.git.changedPaths(
		repoRoot,
		recorded.baselineCommitSha,
		config.discoveryScope,
	);
	const dirtyPaths = await ports.git.dirtyPaths(
		repoRoot,
		config.discoveryScope,
	);
	const changedPaths = [...new Set([...committedPaths, ...dirtyPaths])].sort();
	const pathFootprint = footprint(
		loadedSpec.blocks,
		config.footprint.bindingIdPrefix,
		config.footprint.bindingField,
	);
...
function baseline(
	...
	baselineId: string,
): { freshnessToken: string; baselineCommitSha: string } {
...
async function assertGlobMatches(
	git: RefreshGitPort,
	repoRoot: string,
	...
	for (const entry of scope.filter((value) => /[*?[]/.test(value))) {
		...
		if (paths.length === 0) {
			throw configFailure(
				"config-invalid",
				`discovery_scope glob matched zero files: ${entry}`,
			);
		}
	}
}
