...
export function stubs(paths: readonly string[], footprint: Footprint, emittedAt: string): Stub[] {
  return [...new Set(paths)].sort((left, right) => left.localeCompare(right, "en")).map((path) => {
    ...
    const targetIds = [...new Set(entries.flatMap((entry) => entry.targetIds))].sort();
    return {
      kind: "Delta",
      path,
