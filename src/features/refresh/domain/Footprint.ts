...
export function footprint(
	...
	const entries = blocksByNeutralPrefix(blocks, bindingIdPrefix).map(
		(block) => {
			return {
				impId: block.id,
...
export function impsMissingBinding(
	blocks: readonly SpecBlock[],
	bindingIdPrefix: string,
	bindingField: string,
): string[] {
...
export function footprintEntriesForPath(
	footprint: Footprint,
	path: string,
): FootprintEntry[] {
	return footprint.entries.filter((entry) =>
		entry.paths.some((boundPath) => covers(boundPath, path)),
	);
}
