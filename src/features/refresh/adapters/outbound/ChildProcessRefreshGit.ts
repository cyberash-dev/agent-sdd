...
export class ChildProcessRefreshGit implements RefreshGitPort {
  ...
  async changedPaths(repoRoot: string, baselineCommitSha: string, scope: readonly string[]): Promise<string[]> {
    const result = await runGit(repoRoot, ["diff", "--name-only", `${baselineCommitSha}..HEAD`, "--", ...scope]);
    if (result.code !== 0) {
