...
export class NodeApproveFileSystem implements ApproveConfigPort, ApproveFileSystem {
...
async function walk(dir: string, remaining: string[], acc: string[]): Promise<void> {
  ...
  if (head === "**") {
    ...
    for (const e of entries) {
      if (e.isDirectory()) await walk(join(dir, e.name), remaining, acc);
    }
    await walk(dir, rest, acc);
    return;
  }
  for (const e of entries) {
    if (!matchSegment(head, e.name)) continue;
    const abs = join(dir, e.name);
    if (rest.length === 0) {
      if (e.isFile()) acc.push(abs);
      continue;
    }
    if (e.isDirectory()) await walk(abs, rest, acc);
  }
}
...
async function readConfig(path: string): Promise<string> {
  ...
  catch (error) { throw configFailure("config-missing", ".sdd/config.json is missing or unreadable", errorMessage(error), path); }
}
function parseConfigJson(text: string, path: string): unknown {
  try { return JSON.parse(text) as unknown; }
  catch (error) { throw configFailure("config-invalid", ".sdd/config.json is not valid JSON", errorMessage(error), path); }
}
