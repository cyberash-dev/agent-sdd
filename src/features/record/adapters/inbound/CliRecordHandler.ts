...
import { listRecords, type RecordSummary } from "../../application/ListRecords.js";
import { setRecord, type RecordWritePorts, type RecordWriteResult } from "../../application/SetRecord.js";
...
function writeResult(result: RecordWriteResult, format: Format): CommandResult {
  if (!result.ok) {
    if (format === "json") {
      const body = JSON.stringify({ format_version: 1, ok: false, reason: result.reason, detail: result.message });
      return { exitCode: result.exitCode, stdout: `${body}\n`, stderr: "" };
    }
    return { exitCode: result.exitCode, stdout: "", stderr: `${result.reason}: ${result.message}\n` };
  }
  if (format === "json") {
    return ok(JSON.stringify({
      format_version: 1,
