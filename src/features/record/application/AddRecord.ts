...
import { isProtectedStatus, type RecordWriteResult } from "../domain/RecordWrite.js";
...
export async function addRecord(cwd: string, afterId: string, rawBody: string, ports: WritePorts): Promise<RecordWriteResult> {
  const body = normalizeBody(rawBody);
  const facts = inspectBody(body);
  if (facts.id === null) return invalid("body has no id or does not parse as YAML");
  ...
  if (anchors.length === 0) return notFound(`anchor not found: ${afterId}`);
  if (anchors.length > 1) return ambiguous(`anchor id is ambiguous: ${afterId} matches ${anchors.length} records`);
  ...
  if (fenceClose === -1) return invalid(`could not locate the fence enclosing anchor ${afterId}`);

  const bodyLines = body.split("\n");
  const block = ["", "```yaml", "---", ...bodyLines, "---", "```"];
  const newLines = [...lines.slice(0, fenceClose + 1), ...block, ...lines.slice(fenceClose + 1)];
  await ports.writer.writeSpecFile(cwd, anchor.entry.path, newLines.join("\n"));

  const startLine = fenceClose + 1 + 3 + 1; // ``` (1-based) + blank + ```yaml + --- → first body line
  return { ok: true, action: "add", id: facts.id, file: anchor.entry.path, startLine, endLine: startLine + bodyLines.length };
}
