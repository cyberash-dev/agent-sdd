...
export async function setRecord(cwd: string, id: string, rawBody: string, ports: RecordWritePorts): Promise<RecordWriteResult> {
  const body = normalizeBody(rawBody);
  const facts = inspectBody(body);
  if (facts.id === null) return invalid("body has no id or does not parse as YAML");
  if (facts.id !== id) return invalid(`body id (${facts.id}) does not match target id (${id})`);
  ...
  if (hits.length === 0) return notFound(`record not found: ${id}`);
  if (hits.length > 1) return ambiguous(`record id is ambiguous: ${id} matches ${hits.length} records`);
  const hit = hits[0]!;
  ...
  const newLines = [...lines.slice(0, hit.startLine - 1), ...bodyLines, ...lines.slice(hit.endLine - 1)];
  await ports.writer.writeSpecFile(cwd, hit.entry.path, newLines.join("\n"));

  return { ok: true, action: "set", id, file: hit.entry.path, startLine: hit.startLine, endLine: hit.startLine + bodyLines.length };
}
