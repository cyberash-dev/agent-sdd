...
import {
	...
	type DoctorOutcome,
	type RunDoctorPorts,
} from "../../application/RunDoctor.js";
import type {
	DoctorCommand,
	DoctorRequest,
} from "../../ports/inbound/DoctorCommand.js";
...
export class CliDoctorHandler implements DoctorCommand {
	...
	async execute(
		...
		if (!req.ruleVersion) {
			...
			return refusal(
				env,
				"doctor requires --rule-version (no other modes implemented in v0.4)",
				2,
				format,
			);
		}
...
function render(
	...
	if (outcome.kind === "drift") {
		...
		const lines: string[] = [];
		lines.push(
			`doctor: drift detected (rule_version=${outcome.ruleVersion}, cli_version=${outcome.cliVersion}, compatible_range=${outcome.compatibleRange})`,
		);
