...
export class CliPlanShowHandler implements PlanShowCommand {
  ...
  async execute(cwd: string, req: PlanShowRequest, format: Exclude<OutputFormat, "yaml">): Promise<CommandResult> {
    try {
      const lookup = await showPlan(cwd, req.planId, this.ports);
      return renderLookup(lookup, format);
    } catch (error) {
      if (error instanceof CliFailure) return failed(error, format);
      throw error;
    }
  }
}

function renderLookup(lookup: PlanLookup, format: Exclude<OutputFormat, "yaml">): CommandResult {
      ...
      return ok(JSON.stringify({
        ...
        plan: {
          ...
          pending_attestations: lookup.plan.pendingAttestations.map((a) => ({
            ...
            ...(a.reviewedTestOracle !== undefined ? { reviewed_test_oracle: a.reviewedTestOracle } : {}),
          })),
        },
      }));
    }
...
function refusal(envelope: object, human: string, format: Exclude<OutputFormat, "yaml">): CommandResult {
  if (format === "json") {
