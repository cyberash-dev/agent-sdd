...
export class NodePlanReader implements PlanReader, PlanConfigPort {
  ...
  async read(repoRoot: string, plansDir: string, planId: string | undefined): Promise<PlanLookup> {
    ...
    try {
      ...
      return { kind: "invalid-plan-file", planId: id, sourcePath, reason: "plan file not found" };
    }
    let parsed: unknown;
    try {
    ...
    try {
      ...
      return { kind: "invalid-plan-file", planId: id, sourcePath, reason: (e as Error).message };
    }
  }
}
