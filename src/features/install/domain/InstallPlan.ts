...
function planClaude(
  ...
  for (const artifact of artifactsFor(manifest, "claude")) {
    ...
    writes.push({ absPath: homePath(home, relPath), content: content(sources, artifact.source), executable: artifact.kind === "hook" });
    actions.push({ target: "claude", kind: artifact.kind, op: "copy", path: relPath, note: null });
    ...
    if (artifact.kind === "hook") {
      ...
      desiredHooks.push({ matcher: artifact.event!, command });
      actions.push({ target: "claude", kind: "hook", op: "merge_hook", path: ".claude/settings.json", note: artifact.event! });
    }
  }

  const claudeMdRel = ".claude/CLAUDE.md";
  writes.push({ absPath: homePath(home, claudeMdRel), content: upsertManagedBlock(existing.claudeMd, claudeImportBody(contextSources)), executable: false });
  actions.push({ target: "claude", kind: "managed_block", op: "write_block", path: claudeMdRel, note: null });

  if (desiredHooks.length > 0) {
    writes.push({ absPath: homePath(home, ".claude/settings.json"), content: mergeHooks(existing.settingsJson, desiredHooks), executable: false });
  }
...
function planCodex(
  ...
  for (const artifact of artifactsFor(manifest, "codex")) {
    ...
    writes.push({ absPath: homePath(home, relPath), content: content(sources, artifact.source), executable: false });
    actions.push({ target: "codex", kind: artifact.kind, op: "copy", path: relPath, note: null });
    if (artifact.kind === "context") {
      contextSources.push(artifact.source);
    }
  }
  ...
  for (const artifact of manifest.artifacts) {
    if (artifact.kind === "hook" && !artifact.targets.includes("codex")) {
      actions.push({ target: "codex", kind: "hook", op: "skip", path: artifact.source, note: "codex has no PreToolUse host" });
    }
  }

  const agentsMdRel = ".codex/AGENTS.md";
  writes.push({ absPath: homePath(home, agentsMdRel), content: upsertManagedBlock(existing.agentsMd, codexReferenceBody(contextSources)), executable: false });
  actions.push({ target: "codex", kind: "managed_block", op: "write_block", path: agentsMdRel, note: null });

  return { writes, actions };
}

function artifactsFor(manifest: RuleManifest, agent: AgentTarget): ManifestArtifact[] {
