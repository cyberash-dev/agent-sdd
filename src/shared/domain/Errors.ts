...
export function configFailure(reason: FailureReason, message: string, detail?: string, path?: string): CliFailure {
  return new CliFailure(2, reason, message, detail, path);
}

export function environmentFailure(reason: FailureReason, message: string, detail?: string): CliFailure {
