...
export function parsePlanFile(value: unknown): PlanFileShape {
	...
	if (!PLAN_ID_GRAMMAR.test(planId)) {
		throw new Error(
			`plan_id "${planId}" does not match grammar ${PLAN_ID_GRAMMAR.source}`,
		);
	}
	...
	const pending = pendingRaw.map(
		(item: unknown, idx: number): PendingAttestation => {
			if (!isObj(item)) {
			...
			if (!isTargetStatus(ts)) {
				throw new Error(
					`pending_attestations[${idx}].target_status invalid: ${ts}`,
				);
			}
...
function isTargetStatus(v: string): v is AttestationTargetStatus {
