...
export function blocksByNeutralPrefix(
	blocks: readonly SpecBlock[],
	prefix: string,
): SpecBlock[] {
...
export function stringValue(
	...
	if (typeof value !== "string" || value.length === 0) {
		throw configFailure(
			"config-invalid",
			`${block.id}.${field} must be a non-empty string`,
			undefined,
			specPath,
		);
	}
...
function specBlock(document: ParsedDocument): SpecBlock {
	...
	if (parsed.errors.length > 0) {
		...
		throw configFailure(
			"config-invalid",
			`invalid YAML block at line ${document.line}`,
			message,
		);
	}
	...
	if (!isRecord(value)) {
		throw configFailure(
			"config-invalid",
			`YAML block at line ${document.line} must be an object`,
		);
	}
	const id = value.id;
	...
	if (typeof id !== "string" || typeof type !== "string") {
		throw configFailure(
			"config-invalid",
			`YAML block at line ${document.line} must include string id and type`,
		);
	}
