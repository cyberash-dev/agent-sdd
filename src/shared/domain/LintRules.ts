...
export function baselineVersionRequiredRule(rec: LintRecord): Diagnostic[] {
  if (rec.template !== "Delta" && rec.template !== "Migration") return [];
...
export function deprecatedFieldsRequiredRule(rec: LintRecord): Diagnostic[] {
  if (rec.lifecycleStatus !== "deprecated") return [];
  const out: Diagnostic[] = [];
 ...
 *
 *  Note: methodology canonical for the trigger predicate is `blocking ==
 *  "advisory"` per the upstream Plan 2 literal. The current sdd-cli spec uses
 *  blocking=no/yes; OQ-018 tracks whether `no` should also fire. */
export function assumptionDowngradeApprovalRule(
  rec: LintRecord,
  approverBlocklist: readonly string[] = [],
): Diagnostic[] {
  if (rec.template !== "ASSUMPTION") return [];
  const blocking = rec.parsed.blocking;
...
function readApproverIdentity(rec: LintRecord): string | null {
  ...
  if (typeof lifecycle === "object" && lifecycle !== null && !Array.isArray(lifecycle)) {
    const nested = (lifecycle as Record<string, unknown>).approval_record;
    if (typeof nested === "object" && nested !== null && !Array.isArray(nested)) {
      const id = (nested as Record<string, unknown>).approver_identity;
      if (typeof id === "string" && id.length > 0) return id;
    }
  }
  const flat = rec.parsed.approval_record;
  if (typeof flat === "object" && flat !== null && !Array.isArray(flat)) {
    const id = (flat as Record<string, unknown>).approver_identity;
    if (typeof id === "string" && id.length > 0) return id;
  }
  return null;
}
...
export function generatedArtifactSurfaceRefRule(rec: LintRecord): Diagnostic[] {
  if (rec.template !== "GeneratedArtifact") return [];
  if (rec.parsed.published_surface !== "yes") return [];
...
export function boundaryPolicyRefRule(rec: LintRecord, boundaryIds: ReadonlySet<string>): Diagnostic[] {
  if (!boundaryIds.has(rec.id)) return [];
  const refs = rec.parsed.policy_refs;
  if (Array.isArray(refs) && refs.length > 0) return [];
  if (isObject(refs) && typeof (refs as Record<string, unknown>).not_applicable === "string") return [];
  const override = rec.parsed.policy_override;
  if (isObject(override) && typeof (override as Record<string, unknown>).rationale === "string") return [];
  return [{
    severity: "error",
 ...
 *  required sub-fields. */
export function boundaryConcurrencyModelRule(rec: LintRecord, boundaryIds: ReadonlySet<string>): Diagnostic[] {
  if (!boundaryIds.has(rec.id)) return [];
  const cm = rec.parsed.concurrency_model;
  if (isObject(cm) && typeof (cm as Record<string, unknown>).not_applicable === "string") return [];
  if (!isObject(cm)) {
    return [{
      severity: "error",
...
export function applicabilityRequiredRule(rec: LintRecord, boundaryIds: ReadonlySet<string>): Diagnostic[] {
  if (!boundaryIds.has(rec.id)) return [];
  const a = rec.parsed.applicability;
  if (isObject(a) && Object.keys(a).length > 0) return [];
  return [{
    severity: "error",
...
/** ENF-016: boundary CTR/BEH that touches persistent state must declare
 *  data_scope. We use a heuristic for "touches persistent state": presence of
 *  ANY of `schema`, `external_identifiers`, `members`, OR a record whose
 *  template is Behavior with surface_ref pointing to a public_db /
 *  public_storage Surface. The rule allows `data_scope.not_applicable` with
 *  an explicit reason. */
export function dataScopeRequiredRule(rec: LintRecord, boundaryIds: ReadonlySet<string>): Diagnostic[] {
  if (!boundaryIds.has(rec.id)) return [];
  const ds = rec.parsed.data_scope;
  if (typeof ds === "string" && ds.length > 0) return [];
  if (isObject(ds) && typeof (ds as Record<string, unknown>).not_applicable === "string") return [];
  return [{
    severity: "error",
